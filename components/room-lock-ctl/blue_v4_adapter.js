...
class BlueV4Adapter {
  ...
  _send_buffer() {
    ...
    if (this.open_door_commands.length > 0) {
      ...
      this.connect_callback && this.connect_callback('block_by_open_door_command 暂不发送');
    ...
    wx.getBluetoothAdapterState({
      success: (res) => {
        ...
        if (isDvailable) {
          ...
          if (!id || !time || (id.length < 16 || time < Math.round(new Date().getTime() / 1000) - (3600 * 24))) {
  ...
  startBluetoothDevicesDiscovery() {
    ...
    wx.startBluetoothDevicesDiscovery({
      ...
      fail: (res) => {
        this.connect_callback && this.connect_callback('start_device_discovery_error');
      }
    });
  ...
  onBluetoothDeviceFound() {
    wx.onBluetoothDeviceFound((res) => {
      res.devices.forEach((device) => {
        ...
        if (!(device.name.slice(0, 2) == "FG") && !(device.name.slice(0, 5) == "MESH-")) {
          return
        }
  ...
  onConnectChange() {
    wx.onBLEConnectionStateChange((res) => {
      DEBUG && console.log(`device ${res.deviceId} state has changed, connected: ${res.connected}`)
    ...
    wx.getBLEDeviceServices({
      ...
      success: (res) => {
        ...
        res.services.forEach((service) => {
          if (service.uuid.split('-')[0] == "00001800" || service.uuid.split('-')[0] == "00001801") {
          } else {
