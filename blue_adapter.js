...
class BlueAdapter {
    ...
    wx.getBluetoothAdapterState({
      success: (res) => {
        ...
        if (isDvailable) {
          ...
          } else if (time && time < Math.round(new Date().getTime() / 1000) - (3600 * 24)) {
            console.log('开始连接--------------')
            this.startBluetoothDevicesDiscovery();
          } else {
  ...
  storeOpenCommandId(command) {
    ...
    if (command.slice(0, 2) == "BA" && (command.slice(10, 12) == "01" || command.slice(10, 12) == "02")) {
  ...
  _dealWithV14(res) {
    ...
    if (value.slice(0, 2) == "ce" && value.slice(12, 14) == "e1" && this.activev20 == false) {
      this.activev20 = true
      console.log('需要激活-------------')
      setTimeout(() => {
        this.activev20 = false
    ...
    if (value.slice(0, 4) == 'ceba' && value.slice(4, 12) == this.open_command_id) {
